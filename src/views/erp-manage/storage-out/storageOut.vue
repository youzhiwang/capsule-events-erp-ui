    ...
    <div class="storage-out">
        <div class="filter-wrap">
            <el-form ref="FilterForm" :model="filterForm" label-width="80px">
                ...
                <el-row :gutter="24">
                    <el-col :span="6">
                        <el-form-item label="销库时间" prop="storageOutTime">
                            <el-date-picker v-model="filterForm.storageOutTime" value-format="yyyy-MM-dd"
                                            placeholder="请选择销库时间" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                    </el-col>
        ...
        <div class="operation-wrap">
            <el-button type="primary" @click="addDialogVisible = true">销 库</el-button>
        ...
        <div class="table-wrap">
            <el-table :data="tableData" stripe border height="100%">
                ...
                <el-table-column label="销库数量" prop="storage_out_number" />
                <el-table-column label="销库时间" prop="storage_out_time">
                    <template #default="{row}">
                        <span>{{ row.storage_out_time | timeFormat }}</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="销库操作"
            ...
            <el-form ref="AddForm" :model="addForm" :rules="addRules" label-width="80px">
                ...
                <el-form-item label="销库数量" prop="storageOutNumber">
                    <el-input v-model="addForm.storageOutNumber" placeholder="请输入销库数量" type="number"/>
                </el-form-item>
                <el-form-item label="备注" prop="remark">
                    <el-input v-model="addForm.remark" type="textarea" placeholder="请输入备注"/>
                </el-form-item>
            </el-form>
            <el-alert title="销库操作无法撤销，请仔细核对！" type="error" :closable="false"/>
    ...
    methods: {
      ...
      handleAddConfirm() {
        this.$refs.AddForm.validate(v => {
          if (v) {
            this.axios.post('/api/storage-out', this.addForm).then(res => {
              res = res.data
              if (res.code === 'success') {
                this.handleAddCancel()
                this.getTableData()
                this.$message.success('添加销库记录成功')
      ...
      getTableData() {
        ...
        this.axios.get('/api/storage-out', {
          params
        }).then(res => {
          res = res.data
          if (res.code === 'success') {
